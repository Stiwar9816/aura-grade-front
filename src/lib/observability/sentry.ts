...
const sensitiveKey =
	/authorization|cookie|password|token|secret|credential|recovery|api.?key|otp|document/i;
...
export const sanitizeSentryEvent = <T extends Event | ErrorEvent>(
	event: T,
): T => {
	if (event.request) {
...
const classifyFeature = (path: string): string => {
	if (
		/\/(login|register|forgot-password|reset-password|create-password)|\/api\/auth\b/.test(
			path,
		)
	) {
		return "auth";
