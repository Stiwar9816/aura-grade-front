...
export const RubricLibrary: React.FC<RubricLibraryProps> = ({
  ...
  const handleDelete = async (
    ...
    const confirmed = await confirm({
      ...
      message: `Se eliminará la rúbrica "${title}". Esta acción no se puede deshacer.`,
      confirmLabel: "Eliminar rúbrica",
      tone: "danger",
    });
    if (confirmed) onDeleteTemplate(id);
  };
          ...
          {filteredTemplates.map((template: RubricTemplate) => (
            ...
            >
              ...
              <div className="mt-auto pt-4 border-t border-gray-100 grid grid-cols-2 gap-3">
                <button
                  onClick={(e) =>
                    void handleDelete(e, template.id, template.title)
                  }
